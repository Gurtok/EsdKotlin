            ...
            if (System.currentTimeMillis() > globalUploadTimer + 5000) {
                ...
                if (!checkForElasticHost()) {
                    ...
                    if (!nextString.isEmpty()) {
                        Log.e("DbHelper", "String was not empty!")
                        esIndexer.setNextString(nextString)
                    } else {
                        Log.e("DbHelper", "String was empty!")
                        return
                    }
                    try {
                        Log.e("ES Index", "Trying to index")
                        uploadSuccess = false
                        // Start the indexing thread, and join to wait for it to finish.
                        Log.e("ES Index", "Trying to start thread")
                        esIndexer.run()
                        Log.e("ES Index", "Index Done!")
